...
def checksum1(st):
    return int(reduce(lambda x,y:x+y, map(ord, str(st))))
...
def traceroute(destination_server, count_of_packets, packet_size, max_hops, timeout):
...
if __name__ == '__main__':
    ...
    destination_server = str(input("enter the destination server ip"))
    timeout = int(input("Enter the timeout"))
    packet_size = int(input("Enter the Packet Size"))
    count = int(input("enter the no of packets"))
    max_hops = int(input("enter the no of hops"))
